<?php
//echo '<textarea class="code">'; var_dump($events); echo '</textarea>';

if(isset($events['Events'])) $events = $events['Events'];

if(count($events)==0 && isset($start)) {
  $first_date = $start;
} else {
  $first_date = (isset($events[0])) ? strtotime($events[0]['start']) : strtotime('now');
}
